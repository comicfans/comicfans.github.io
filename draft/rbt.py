...
class AniNode:
    def __init__(self, ani_context, tree_node):
        ...
        for value in Dir:
            ...
            lambda: Line(self.group_node.get_bottom(),
                         self.group_node.get_bottom() if not self.tree_node.children_[value.value] else self.ani_context.node_for(self.tree_node.children_[value.value].value).group_node.get_top())
        ))
            self.ani_context.scene.add(self.children_edges[-1])
        

    def set_color(self, color: Color,animation : list[Animation]):
        target_color = manim.GRAY if color == Color.BLACK else manim.RED

        for obj in [self.circle,self.text]:
            animation.append(obj.animate.set_color(target_color))
...
class Node:
    ...
    def set_color(self, color: Color, animation: list[Animation]):
        ...
        self.color_ = color
        return self.ani_node().set_color(color, animation)


    def set_child(self, dir: Dir, node, animation: list[Animation]):
...
class RBT:
    def __init__(self, ani_context: AniContext):
        self.root = None
        self.ani_context = ani_context
        width,height = RBT.rect_width_height(0)
        self.rect = Rectangle(width = width, height = height).move_to(RBT.rect_pos(height))
        ani_context.scene.add(self.rect)

    def find_value_pos(self, root, value)-> Tuple[Node, dir]:
        animation = []
        parent = None
        next_try = self.root
        child_dir = None
    ...
    def depth(node: Node)->int:
        ...
        return 1+max(RBT.depth(node.children_[Dir.LEFT.value]),
                     RBT.depth(node.children_[Dir.RIGHT.value]))

    def assign_position(self, root, offset, total_depth, current_depth, dir:Dir| None, animation):
        if not root:
            return None

        horizontal_base = offset + (0 if not dir else pow(2, total_depth - current_depth))
    ...
    def node_position_animation(self, animation):
        self_depth = RBT.depth(self.root)
        # root sit at 0,0
        self.assign_position(self.root, 0, self_depth, 0, None, animation)


        width,height = RBT.rect_width_height(self_depth)

        animation.append(self.rect.animate.move_to(RBT.rect_pos(height)))
    ...
    def insert(self, value)->bool:
        ...
        parent,dir = self.find_value_pos(self.root, value)
        ...
        while new_node:
            ...
            if uncle.color == Color.BLACK:
                ...
                #  /  \    /  \               /   \
                #L(R) S(B) .  .. ..         S(B)   U(B)
                grandparent.rotate(1 - parent_dir)
